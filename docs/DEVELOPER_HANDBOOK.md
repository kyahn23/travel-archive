...
> **부분적으로 오래된 문서 (2026-08-18)**: 제품/구조 참고에는 사용할 수 있지만 DB와 배포 절은 현재 작업 트리보다 오래됐다. 현재는 `SeedDataLoader`가 삭제되고 Flyway/분리 Compose로 전환 중이며 배포 gate도 통과하지 않았다. 운영 판단은 `docs/README.md`를 우선한다.
